...
class NormalizeTransactionTest < Minitest::Test
  ...
  def run_failing_normalization(source_one, source_two, failure_command)
    @commands = []
    SecureRandom.stub(:hex, "token") do
      stub_cloud(@commands, source_one, source_two, failure_command) do
        Cloud::Normalize.call("project", "bucket")
      end
    end
  end

  def stub_cloud(commands, source_one, source_two, failure_command, &)
    Cloud::Normalize.stub(:sleep, nil) do
      Cloud::ObjectMove.stub(:move, failing_move(commands, failure_command)) do
  ...
  def listing_pipe(source_one, source_two)
    ->(_command, &block) { block.call(StringIO.new("#{source_one}\n#{source_two}\n")) }
  end

  def failing_move(commands, failure_command)
    failed = false
    lambda do |source, target|
      command = Cloud::ObjectMove.command(source, target)
      commands << command
      next "101" unless command == failure_command && !failed

      failed = true
      raise StandardError, "move failed"
    end
  end
